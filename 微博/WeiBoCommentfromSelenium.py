...
import hashlib
from proxyServer import ProxyServerBase
...
def parseComment(tmpHtml, weiboTest3, url):
    soup = etree.HTML(tmpHtml)
    divs = soup.xpath('//div[@node-type="comment_list"]/div')
    for div in divs:
        item = {}
        try:
            item[u'text'] = div.xpath(u'.//div[@class="WB_text"]')[0].xpath('string(.)').strip()
            item[u'url'] = url
            weiboTest3.insert(item)
        except:
            continue
        print u'i got you!!!!!!!!!!!'

def loginWeibo():
    p = ProxyServerBase(u'creditchina_gov_cn')
    myProxy = p.get().values()[0].split('/')[-1]
    proxy = Proxy(
                {
...
def saveCommentPage(url, resource):
    fileName = u'./comment/' + hashlib.md5(url).hexdigest() + u'.html'
    with codecs.open(fileName, u'w', u'utf-8') as f:
        f.write(resource)
...
db = conn[u'test']
weiboTest4 = db[u'weiboTest4']
weiboTest5 = db[u'weiboTest5']
browser = loginWeibo()

lists = weiboTest4.find()


for i in lists:
    try:
        url = i[u"链接地址"]
        if not weiboTest5.find({u'url': url}).count():
            browser = getPage(browser, url)
            tmpHtml = browser.page_source
            saveCommentPage(browser.current_url, tmpHtml)
            parseComment(tmpHtml, weiboTest5, url)
            while True:
                browser.find_element_by_xpath('//a[contains(@class, "page next")]/span').click()
                time.sleep(5)
                tmpHtml = browser.page_source
                parseComment(tmpHtml, weiboTest5, url)
    except Exception, e:
...
# browser = getPage(browser, u'http://s.weibo.com/weibo/%25E8%25BD%25AC%25E5%259F%25BA%25E5%259B%25A0%2520%25E9%25A3%259F%25E5%2593%2581&Refer=STopic_box')
# while True:
